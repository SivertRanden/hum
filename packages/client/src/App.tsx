...
export default function App() {
  ...
  // Check for password reset token in URL — stored in state so clearing it triggers a re-render
  ...
  if (!auth) {
    if (resetToken) {
      return <ResetPasswordScreen token={resetToken} onDone={() => {
        // Clear the token from the URL and trigger re-render to show login
        window.history.replaceState({}, '', window.location.pathname);
        setResetToken(null);
      }} />;
    }
