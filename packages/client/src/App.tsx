...
export default function App() {
  ...
  useEffect(() => {
    ...
    const oauthUserId = urlParams.get('oauth_userId');
    if (oauthToken && oauthUsername && oauthUserId) {
      const authData = { token: oauthToken, username: oauthUsername, userId: Number(oauthUserId) };
      localStorage.setItem('hum_auth', JSON.stringify(authData));
      setAuth(authData);
